...
#  for cifar only import has to be changes
import code_.networks.efficient_net.efficientnet_model_cifar as efficientnet_model
...
def build_model(images,
    ...
    #  removed this part -> don't use any overide_params so removing this part should be fine. + training is a tensor in our case
    # if not training or fine_tuning:
    #   if not override_params:
    #     override_params = {}
    #   override_params['batch_norm'] = utils.BatchNormalization
    # override_params={} # TODO remove only for debug
    # override_params['drop_connect_rate']=0.0
    ...
    with tf.variable_scope(model_name):
        ...
        outputs, reuse_binary_tensor = model(images, training=training, features_only=features_only)
    outputs = tf.identity(outputs, 'features' if features_only else 'logits')
    return outputs, model.endpoints, reuse_binary_tensor
